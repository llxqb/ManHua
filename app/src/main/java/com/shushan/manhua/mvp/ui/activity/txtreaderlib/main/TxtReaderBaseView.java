...
import com.shushan.manhua.mvp.ui.activity.txtreaderlib.bean.DefaultLeftSlider;
import com.shushan.manhua.mvp.ui.activity.txtreaderlib.bean.DefaultRightSlider;
import com.shushan.manhua.mvp.ui.activity.txtreaderlib.bean.Slider;
import com.shushan.manhua.mvp.ui.activity.txtreaderlib.bean.TxtChar;
import com.shushan.manhua.mvp.ui.activity.txtreaderlib.bean.TxtLine;
...
import com.shushan.manhua.mvp.ui.activity.txtreaderlib.interfaces.IPageEdgeListener;
import com.shushan.manhua.mvp.ui.activity.txtreaderlib.interfaces.ISliderListener;
import com.shushan.manhua.mvp.ui.activity.txtreaderlib.interfaces.ITxtLine;
import com.shushan.manhua.mvp.ui.activity.txtreaderlib.interfaces.ITxtTask;
import com.shushan.manhua.mvp.ui.activity.txtreaderlib.tasks.BitmapProduceTask;
import com.shushan.manhua.mvp.ui.activity.txtreaderlib.tasks.TextLoader;
import com.shushan.manhua.mvp.ui.activity.txtreaderlib.tasks.TxtFileLoader;
import com.shushan.manhua.mvp.ui.activity.txtreaderlib.utils.DisPlayUtil;
...
public abstract class TxtReaderBaseView extends View implements GestureDetector.OnGestureListener {
     ...
     * @param event
     */
    protected void startPageUpAnimation_Y(MotionEvent event) {
        if (getMoveDistanceY() < -PageChangeMinMoveDistance || getMoveDistanceY() > PageChangeMinMoveDistance) {
            if (isPagePre()) {
                if (!isFirstPage()) {
                    startPagePreAnimation();
                } else {
            ...
            //没有超出距离，自动还原
            if ((getMoveDistanceY() > 0 && isFirstPage()) || (getMoveDistanceY() < 0 && isLastPage())) {
                //这种情况不执行
            } else {
                //如果只是移动一点点，释放即可，不需要恢复
                if ((getMoveDistanceY() > 0 && getMoveDistanceY() < 5) || (getMoveDistanceY() <= 0 && getMoveDistanceY() > -5)) {
                    releaseTouch();
                    invalidate();
                } else {//ss
                    startPageStateBackAnimation();
    ...
    private boolean dealCenterClickAndDoChangePage(MotionEvent e) {
        ...
        if (CurrentMode == Mode.Normal && readerContext.InitDone()) {
            ...
            boolean needPagePreY = y < top;
            boolean needPageNextY = y > bottom;
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        ...
        if (CurrentMode == Mode.Normal) {//正常情况下快速滑动，执行翻页动作
            ...
            if (getOrientation() == 1) {
                if (isPagePre() && !isFirstPage() && velocityY > MaxVelocityY) {
                    startPagePreAnimation();
                    return true;
                } else if (isPageNext() && !isLastPage() && velocityY < -MaxVelocityY) {
                    startPageNextAnimation();
                    return true;
                }
            }
    ...
    private void initReaderContext() {
        if (getOrientation() == 0) {
            ...
            PageChangeMinMoveDistance = getHeight() / 5;
        }
