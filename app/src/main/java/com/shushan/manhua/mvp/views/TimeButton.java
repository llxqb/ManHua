...
public class TimeButton extends android.support.v7.widget.AppCompatButton implements View.OnClickListener {
	...
	private int afterBg;
	private int beforBg;
	private int afterTextBg;
	private int beforTextBg;

	@SuppressLint({ "HandlerLeak" })
	Handler han = new Handler() {
		public void handleMessage(Message paramMessage) {
			TimeButton.this.setText(TimeButton.this.time / 1000L + TimeButton.this.textafter);
			TimeButton.this.setBackgroundResource(TimeButton.this.afterBg);//注释掉保留原背景
			...
			if (TimeButton.this.time < 0L) {
				...
				TimeButton.this.setTextColor(TimeButton.this.beforTextBg);
	...
	public void setAfterTextBg(int paramInt) {
		this.afterTextBg = paramInt;
	}

	public void setBeforTextBg(int paramInt) {
		this.beforTextBg = paramInt;
	}
