package com.shushan.manhua.ireader.widget.page;
...
import com.shushan.manhua.ireader.utils.RxUtils;
...
public abstract class PageLoader {
    ...
    public void saveRecord() {
        ...
        Log.e("ddd", "mBookRecord:" + new Gson().toJson(mBookRecord));
        //存储到数据库
        ...
        if (mStatus != STATUS_FINISH) {
            ...
            switch (mStatus) {
                case STATUS_LOADING:
                    ...
                    break;
                case STATUS_ERROR:
                    tip = "Gagal memuat";//加载失败(点击边缘重试)
                    break;
                case STATUS_EMPTY:
                    tip = "Konten artikel kosong";//文章内容为空
                    break;
                case STATUS_PARING:
                    tip = "Harap tunggu untuk pengaturan huruf...";//正在排版请等待
                    break;
                case STATUS_PARSE_ERROR:
                    tip = "error";//文件解析错误
                    break;
                case STATUS_CATEGORY_EMPTY:
                    tip = "Daftar direktori kosong";//目录列表为空
                    break;
            }
    ...
    private void dealLoadPageList(int chapterPos,boolean isSizeChange) {
        ...
        // 回调
        chapterChangeCallback(isSizeChange);
    }

    private void chapterChangeCallback(boolean isSizeChange) {
        if (mPageChangeListener != null) {
            mPageChangeListener.onChapterChange(mCurChapterPos,isSizeChange);
...
//                    paragraph = paragraph.replaceAll("\\s", "");
    ...
    public interface OnPageChangeListener {
         ...
         *           isSizeChange 文字大小改变
         */
        void onChapterChange(int pos,boolean isSizeChange);
         ...
         * 作用：返回当前章节数据回调
         */
        void onPageUnReadble(TxtChapter chapter);
    }
}
